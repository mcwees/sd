#!/usr/bin/perl
#
# Sending monthly call-agent activity report
# Started at 15.01.2026 by mcwees
#
use strict;
use warnings;
...
sub daily_report (){
 use vars qw($daily $mon $s $end);
# Calc last month
 $s = DateTime->now( time_zone => 'local' );
 $s->set_time_zone('floating');
 $s->truncate( to => 'day' );
 $s->subtract( months => 1 );
 $s->set( day => 1 );
 $mon = "'" . $s->strftime('%Y-%m-%d') ."'";
 $s = DateTime->now( time_zone => 'local' );
 $s->set_time_zone('floating');
 $s->set( day => 1 );
 $end = "'" . $s->strftime('%Y-%m-%d') ."'";
 $daily =<<THEAD;
<h1>Отчет о завяках за период с $mon до $end</h1>
...
SELECT case_name, ext_name, to_char(created_at, 'DD Mon YY HH24:MI:SS')
  ...
  WHERE role = 'call_agent'
    AND t1.created_at > CONCAT($mon, ' 09:00')::timestamp
    AND t1.created_at <= CONCAT($end, ' 09:00')::timestamp
  ORDER BY created_at
DAILY
 $sth = &sql_exec($q);
